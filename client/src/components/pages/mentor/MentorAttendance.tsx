...
import { getAllStudents, getAllSessions, getMentorBatches } from "@/api/mentorApis";
...
// Batches are now stored as a simple string array (e.g., ["A26", "B27"])
// as returned by the getMentorBatches API
...
const MentorAttendance = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const [studentsData, sessionsData, batchesData] = await Promise.all([
        getAllStudents(),
        getAllSessions(),
        getMentorBatches(),
      ]);
  ...
  const handleSubmit = async () => {
    ...
    const batchStudents = students.filter(
      (student) => student.batchId === selectedSessionData.batchId,
    );

    // Check if all students in the batch have a status selected
    const batchStudentIds = new Set(batchStudents.map((s) => s._id));
    const unselectedStudents = attendance.filter(
      (a) => batchStudentIds.has(a.studentId) && !a.status,
    );
  ...
  const getStatusBadge = (status: "present" | "absent" | "late") => {
    const variants = {
      present: {
        color:
          "bg-green-100 text-green-700 border-green-200 hover:bg-green-200 transition-colors",
        icon: CheckCircle,
      },
      absent: {
        color:
          "bg-red-100 text-red-700 border-red-200 hover:bg-red-200 transition-colors",
        icon: XCircle,
      },
      late: {
        color:
          "bg-yellow-100 text-yellow-700 border-yellow-200 hover:bg-yellow-200 transition-colors",
        icon: Clock,
      },
    };
  ...
  if (loading) {
    return (
      ...
      >
        <Loader2
          className="w-10 h-10 animate-spin text-slate-400"
          aria-hidden="true"
        />
    ...
    <div className="min-h-screen bg-gray-50 p-4 md:p-6 lg:p-8">
      ...
      <div className="text-center mb-8">
        ...
        <p className="text-gray-600">
          Record student attendance for scheduled sessions
        </p>
      </div>
                    ...
                    {filteredSessions.length === 0 ? (
                      ...
                      filteredSessions.map((session) => (
                        <SelectItem key={session._id} value={session._id}>
                          {session.topic || (session.topics ? session.topics.join(", ") : "No Topic")} -{" "}
          ...
          <CardHeader className="border-b border-gray-100 pb-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
              ...
              <p className="text-sm text-gray-500">
                {attendance.filter((a) => a.status).length} of{" "}
                {filteredStudents.length} students marked
              </p>
            </div>
          </CardHeader>
          <CardContent className="p-6">
